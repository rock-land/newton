...
|---|---|---|---|---|
| T-101 | Wire pytest-cov into test suite and establish baseline coverage | server | `pytest --cov=src -q` runs by default; coverage report shows >=80% global; pytest.ini updated with `addopts = --cov=src --cov-report=term-missing`; CLAUDE.md quality gate reflects coverage command | TODO |
